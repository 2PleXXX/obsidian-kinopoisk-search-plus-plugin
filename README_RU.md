...
3. **Улучшенный интерфейс и UX** – Пользовательский интерфейс и опыт взаимодействия были улучшены для более плавной работы.
4. **Лучшая совместимость с Obsidian** - Улучшенная интеграция с функциями Obsidian
5. **Исправления ошибок** - Исправлены незначительные проблемы из оригинальной версии
...
> ⚠️ **Важно:** Различайте URL-переменные (прямые ссылки) и переменные ImageLink (готовые markdown-ссылки)

### URL-переменные (прямые ссылки)

Содержат только URL изображений без markdown-форматирования. Всегда содержат веб-ссылки, даже если изображения загружены локально.

| Переменная      | Описание                                       | Тип | Список | Пример                              |
| --------------- | ---------------------------------------------- | --- | ------ | ----------------------------------- |
| `{{posterUrl}}` | Прямая ссылка на постер фильма                 | URL | ✅     | `https://image.tmdb.org/poster.jpg` |
| `{{coverUrl}}`  | Прямая ссылка на обложку (широкое изображение) | URL | ✅     | `https://image.tmdb.org/cover.jpg`  |
| `{{logoUrl}}`   | Прямая ссылка на логотип фильма                | URL | ✅     | `https://image.tmdb.org/logo.png`   |

### Переменные ImageLink (готовые markdown-ссылки)

Содержат готовые markdown-ссылки для отображения изображений в Obsidian. Используют ссылки на локальные файлы при загрузке изображений, иначе веб-ссылки.

| Переменная            | Описание                           | Тип      | Список | Пример                                  |
| --------------------- | ---------------------------------- | -------- | ------ | --------------------------------------- |
| `{{posterImageLink}}` | Готовая markdown-ссылка на постер  | Markdown | ✅     | `![[poster.jpg]]`                       |
| `{{coverImageLink}}`  | Готовая markdown-ссылка на обложку | Markdown | ✅     | `![](https://image.tmdb.org/cover.jpg)` |
| `{{logoImageLink}}`   | Готовая markdown-ссылка на логотип | Markdown | ✅     | `![[logo.png]]`                         |

### Ключевые различия

-   **URL-переменные**: Всегда содержат оригинальные веб-URL для внешнего доступа
-   **Переменные ImageLink**:
    -   Для загруженных изображений: Используют вики-ссылки `![[filename.jpg]]` (переносимые, работают после перемещения файлов)
    -   Для веб-изображений: Используют markdown-ссылки `![](https://...)` (резервный вариант при неудачной загрузке)
